...
exports.LeaveService = exports.determineMdReviewStatus = exports.determineInitialLeaveStatus = exports.determinePostRelieverStatus = exports.determineHrReviewStatus = void 0;
...
const determinePostRelieverStatus = (employeeRole) => {
    const rank = (0, auth_middleware_1.getRoleRank)(employeeRole || '');
    if (rank >= 92)
        return 'MD_REVIEW';
    if (rank >= 65)
        return 'HR_REVIEW';
...
exports.determinePostRelieverStatus = determinePostRelieverStatus;
const determineInitialLeaveStatus = (employeeRole, hasReliever) => {
    if (hasReliever)
        return 'SUBMITTED';
    return (0, exports.determinePostRelieverStatus)(employeeRole);
};
...
class LeaveService {
     ...
     */
    static async validateAndCountSelectedDays(organizationId, dates) {
        if (!Array.isArray(dates) || dates.length === 0) {
            throw new errors_1.ValidationError('Select at least one day of leave.');
        }
        ...
        for (const raw of dates) {
            ...
            if (Number.isNaN(parsed.getTime())) {
                throw new errors_1.ValidationError(`"${raw}" is not a valid date.`);
            }
            ...
            seen.add(key);
            if (parsed < today) {
                throw new errors_1.ValidationError(`${key} is in the past — leave can only be requested for today or future dates.`);
            }
        ...
        for (const d of normalizedDates) {
            ...
            if (dayOfWeek === 0 || dayOfWeek === 6) {
                throw new errors_1.ValidationError(`${key} is a weekend — only weekdays (Monday–Friday) can be selected for leave.`);
            }
            if (holidaySet.has(key)) {
                throw new errors_1.ValidationError(`${key} is a public holiday and cannot be selected for leave.`);
            }
        }
    ...
    static async requestLeave(organizationId, employeeId, data) {
        ...
        if (relieverId && (!handoverNotes || handoverNotes.trim().length < 10)) {
            throw new errors_1.ValidationError('Please provide instructions for your cover person (at least 10 characters) when assigning a reliever.');
        }
        ...
        if (exactConflict) {
            throw new errors_1.ConflictError(`You already have a leave request (${exactConflict.leaveRequest.status}) covering ${exactConflict.date.toISOString().split('T')[0]}.`);
        }
        ...
        if (boundingBoxConflict) {
            throw new errors_1.ConflictError(`You already have a leave request (${boundingBoxConflict.status}) overlapping ${boundingBoxConflict.startDate.toLocaleDateString()} – ${boundingBoxConflict.endDate.toLocaleDateString()}.`);
        }
     ...
     */
    static async notifyAssignedManagerAndHr(organizationId, employee, leaveDays) {
        if (employee.supervisorId) {
            await (0, websocket_service_1.notify)(employee.supervisorId, '📋 Leave Awaiting Your Review', `${employee.fullName} has requested ${leaveDays} day(s) of leave. Pending your approval.`, 'INFO', '/leave');
        }
        const hrDirectors = await client_1.default.user.findMany({
            where: { organizationId, role: { in: ['HR_DIRECTOR', 'DEV'] }, isArchived: false },
            select: { id: true }
        });
        await Promise.all(hrDirectors.map(hr => (0, websocket_service_1.notify)(hr.id, '👁️ Leave With Manager', `${employee.fullName}'s leave is now with their manager for review — visible to you for monitoring.`, 'INFO', '/leave', false)));
    }
    /**
    ...
    static async respondAsReliever(leaveId, relieverId, accept, comment) {
        ...
        const nextStatus = accept ? (0, exports.determinePostRelieverStatus)(leave.employee.role) : 'RELIEVER_DECLINED';
     ...
     */
    static async managerReview(leaveId, managerId, approve, comment, options) {
        ...
        if (!isPrimaryManager && !isDeptManager && !isHrDirectorOverride) {
            throw new errors_1.ForbiddenError('Unauthorized for Step 1 Manager Review. You must be the direct supervisor, a department manager, or the HR Director (override).');
        }
        ...
        if (isOverride && (!comment || comment.trim().length < 3)) {
            throw new errors_1.ValidationError('A reason is required when the HR Director overrides an unavailable manager.');
        }
        ...
        await (0, websocket_service_1.notify)(leave.employeeId, approve ? '📋 Line Manager Approved' : '❌ Line Manager Rejected', approve
            ...
            : `Management has rejected your leave request${overrideNote}. Reason: ${comment}`, approve ? 'INFO' : 'ERROR', '/leave');
        if (approve) {
    ...
    static async hrValidation(leaveId, hrId, approve, comment) {
        ...
        if (approve && leave.requiresMedicalCertificate && !leave.medicalCertificateUploaded) {
            throw new errors_1.ValidationError('A doctor\'s report must be uploaded before HR can approve this sick leave request.');
        }
        ...
        if (approve && isTerminalAtHR) {
            await pdf_service_1.PdfExportService.generateAndArchiveLeaveApprovalPdf(leaveId, [leave.employeeId, leave.managerId, hrId].filter(Boolean));
        }
        return updated;
    }
    /**
    ...
    static async mdFinalReview(leaveId, mdId, approve, comment) {
        ...
        if (approve) {
            await pdf_service_1.PdfExportService.generateAndArchiveLeaveApprovalPdf(leaveId, [leave.employeeId, hrDirectorId, mdId].filter(Boolean));
        }
        return updated;
    }
