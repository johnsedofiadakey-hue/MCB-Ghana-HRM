...
class AIService {
     ...
     */
    static async generateJobDescription(title, departmentName) {
     ...
     */
    static async getCultureInsights(organizationId) {
        ...
        const [appraisals, leaveTrends, terminations] = await Promise.all([
            client_1.default.performanceReviewV2.findMany({
                where: { organizationId, status: 'COMPLETED' },
                select: { finalScore: true, managerReview: true },
                take: 20,
                orderBy: { createdAt: 'desc' }
            }),
            client_1.default.leaveRequest.count({ where: { organizationId, status: 'APPROVED', createdAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } } }),
            client_1.default.user.count({ where: { organizationId, status: 'TERMINATED', updatedAt: { gte: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000) } } })
        ]);
