...
class AppraisalService {
        ...
        if (selfReview && managerReview) {
            ...
            if (managerReview.summary) {
                try {
                    ...
                    await client_1.prisma.appraisalReview.update({
                        where: { id: managerReview.id },
                        data: { aiCalibrationScore: aiScore }
                    });
                }
                catch (e) {
                    console.warn('[AppraisalService] AI Calibration failed:', e);
                }
            }
        }
    }
     ...
     */
    static async analyzeSentiment(summary, score) {
        const apiKey = process.env.GEMINI_API_KEY;
        if (!apiKey)
            return 100; // Skip if no AI key
     ...
     */
    static async syncKpiScores(packetId, organizationId) {
        const packet = await client_1.prisma.appraisalPacket.findUnique({
            where: { id: packetId, organizationId },
            include: { cycle: true }
        });
        if (!packet)
            return 0;
        const targets = await client_1.prisma.target.findMany({
            where: {
                organizationId,
    ...
    static async getReviewerPackets(userId, organizationId, userRank = 0, canViewAll = false) {
        try {
            ...
            const rawPackets = await client_1.prisma.appraisalPacket.findMany({
                where,
                include: {
            ...
            console.error('[AppraisalService] LIVE FETCH ERROR:', err);
            throw err;
        }
    }
    /**
