...
async function setup() {
    ...
    for (const [name, permissions] of Object.entries(permissions_1.PERMISSION_BUNDLES)) {
    ...
    for (const acc of DEFAULT_ACCOUNTS) {
        ...
        const passwordHash = await bcryptjs_1.default.hash(crypto_1.default.randomBytes(48).toString('base64url'), SALT_ROUNDS);
        ...
        if (!existing || user.mustChangePassword) {
            ...
            if (!sent)
                invitationFailures.push(user.email);
        }
    }
    ...
    if (md && finance)
        await prisma.user.update({ where: { id: finance.id }, data: { supervisorId: md.id } });
    if (md && marketing)
        await prisma.user.update({ where: { id: marketing.id }, data: { supervisorId: md.id } });
    if (md && director)
    ...
    for (const dept of depts) {
        ...
        const memberIds = dept.memberIds.filter((id) => Boolean(id));
        if (memberIds.length) {
