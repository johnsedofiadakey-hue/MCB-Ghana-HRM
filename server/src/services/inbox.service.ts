...
export interface InboxAction {
  ...
  data?: any;
  createdAt: Date;
  /** True when the viewer can only observe this item (e.g. HR Director watching a
   * request that's still with the employee's manager) — not act on it under normal
   * circumstances. The UI should visually de-emphasize these. */
  monitorOnly?: boolean;
}
...
export class InboxService {
  static async getActions(organizationId: string, userId: string): Promise<InboxAction[]> {
    ...
    try {
      ...
      const user = await prisma.user.findUnique({ where: { id: userId }, select: { role: true, departmentId: true } });
